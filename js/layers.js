addLayer("T", {
   ...
   "Upgrades": {
    ...
    clickables:{
        11: {
            ...
            onClick() {player.T.pause = player.T.pause.add(1)
                ...
                player.T.pauseTime = player.T.pauseTime.add(1)
            },
        },
        12: {
            title() {a="+1 Point"
    ...
    buyables: {
        ...
        12: {
            ...
            cost(x) { a= new Decimal(5).pow(x).times(500)
                ...
                if(getClickableState('I',12)==1) a= new Decimal(4).pow(x).times(500)
                return a
             },
            ...
            buyMax() {
                    ...
                    if(getClickableState('I',12)==1) tempBuy = player.points.max(500).div(500).log(4)
					//if (tempBuy.gte(25)) tempBuy = tempBuy.times(625).cbrt();
					let target = tempBuy.plus(1).floor();
					player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].max(target);
			},
    ...
    PPMexp(){a=n(1)
        ...
        if(hasUpgrade('E',81)) a=a.times(1.01)
        if(inChallenge('I',23)) a=a.times(buyableEffect('T',13))
        if(getClickableState('I',21)==1) a=a.times(clickableEffect('I',21))
        if(getClickableState('I',33)==1) a=a.times(1.01)
        if(hasUpgrade('E',202)) a=a.times(upgradeEffect('E',202))
    ...
    PEFmult(){a=n(1)
        ...
        if(hasChallenge('E',22)) a=a.times(challengeEffect('E',22))
...
addLayer("Q", {
    ...
    gainExp() { // Calculate the exponent on main currency from bonuses
        ...
        if(getClickableState('I',22)==1) exp=exp.div(clickableEffect('I',22))
        return exp
    },
    directMult() {mult = n(1)
...
addLayer("ST", {
    ...
    getResetGain(){x=n(10).pow(player.T.points.max(1).div(1e12).log(1e12))
            ...
            if(hasUpgrade('ST',43)) {y=y.max(n(1.5).pow(y))
                if(player.Q.points.gte(75000)) y=n(1.5).pow(74995).times(player.Q.points.sub(74995))
            }
            if(x.lt(1)) x=n(0)
   ...
   "Upgrades": {
    ...
    upgrades: {
        ...
        43: {
            ...
            return a},
            tooltip(){a=''
                if(player.Q.points.gte(70000)) {a='After 75000 QqQe308, this Upgrade will be ineffective!'
                    if(options.Chinese) a='当达到75000 QqQe308后，此升级将失效！'
                }
                return a
            },
        },
...
addLayer("Qi", {
    ...
    doReset(resettingLayer) {
        if (layers[resettingLayer].row == 3) {
            let kept = []
            kept.push('cokecole','buyables','qaqe308','QqQeInf')
            if(hasUpgrade('MT',34)||hasUpgrade('I',24)) kept.push('milestones','clickables')
            layerDataReset(this.layer, kept)
    }
    if (layers[resettingLayer].row == 4) {
    let kept = []
    kept.push('buyables','qaqe308','QqQeInf')
    if(hasUpgrade('I',24)) kept.push('milestones')
    ...
    update(diff){
        ...
        if(getClickableState(this.layer, 14)==1) player.Qi.QqQeInf = player.Qi.QqQeInf.add(tmp.Qi.QqQeInfspeed.times(diff))
    },
    passiveGeneration()
    {
        mult = 0
    ...
    clickables:{
        ...
        14: {
            title(){ a="Make Super-QqQeInfinity"
                if(getClickableState(this.layer, this.id)==1) a='Stop making Super-QqQeInfinity'
                if(options.Chinese){a="超QqQeInfinity"
                if(getClickableState(this.layer, this.id)==1) a='停止超QqQeInfinity'}
                return a
            },
            display() {a= "You are currently"
                if(getClickableState(this.layer, this.id)==0) a=a+' NOT'
                a=a+" making Super-QqQeInfinity"
                if(options.Chinese){a= "你当前正在"
                if(getClickableState(this.layer, this.id)==0) a='你当前不在'
                a=a+"超QqQeInfinity"}
            return a},
            unlocked() {return hasMilestone('Qi', 0)&&hasMilestone('MT',3)&&hasUpgrade('I',84)&&hasUpgrade('cf',51)},
            canClick() {return hasMilestone('Qi', 0)&&hasMilestone('MT',3)&&hasUpgrade('I',84)&&hasUpgrade('cf',51)&&(tmp.Qi.currentActive.lt(tmp.Qi.maxActive)||getClickableState(this.layer, this.id)==1)},
            onClick() {setClickableState(this.layer, this.id, 1-getClickableState(this.layer, this.id))},
        },
    },
    buyables: {
        11: {
            ...
            cost(x) { let a=[n(200),n(1e250),n('1e7500'),n('1e1e8'),n(1e310)]
                return a[x.toNumber()]
             },
            effect(x) {return x},
    ...
    Showdetail() {a=''
        ...
        if(hasMilestone('Qi',0)&&hasMilestone('MT',3)&&hasUpgrade('I',84)&&hasUpgrade('cf',51)) {a =a+"<br/>You have made <h3 style='color: #aee308; text-shadow: 0 0 3px #c2b280'>" + format(player.Qi.QqQeInf) + "</h3> Super-QqQeInfinity.<br/>"
        a=a+"Your effective Super-QqQeInfinity is <h3 style='color: #aee308; text-shadow: 0 0 3px #c2b280'>" + format(tmp.Qi.effQqQeInf) + "</h3>"
        //if(tmp.Qi.effqaqe308.gte(10000)) a=a+'(softcapped)'
        a=a+", raising the hardcap of E19-3 by ^<h3 style='color: #aee308; text-shadow: 0 0 3px #c2b280'>"+format(tmp.Qi.QqQeInfeff[0])+"</h3>, and multiplying the first 3 kinds of Super-man generation speed by <h3 style='color: #aee308; text-shadow: 0 0 3px #c2b280'>"+format(tmp.Qi.QqQeInfeff[1])+"</h3> (based on the effect of QqQeInfinity)" 
        //if(tmp.Qi.qaqe308eff.gte('1e500')) a=a+"(softcapped)" 
        a=a+".<br>Based your QqQeInfinity amount，your QqQeInfinity makes <h3 style='color: #aee308; text-shadow: 0 0 3px #c2b280'>"+ format(tmp.Qi.QqQeInfspeed) +"</h3> Super-QqQeInfinity per second,"
        a=a+" which means making one Super-QqQeInfinity per <h3 style='color: #aee308; text-shadow: 0 0 3px #c2b280'>"+format(n(1).div(tmp.Qi.QqQeInfspeed))+"</h3> seconds.<br/>Super-QqQeInfinity is kept until row 6 reset.<br>"}
        ...
        if(hasMilestone('Qi',0)&&hasMilestone('MT',3)&&hasUpgrade('I',84)&&hasUpgrade('cf',51)) {a =a+"<br/>你超了 <h3 style='color: #aee308; text-shadow: 0 0 3px #c2b280'>" + format(player.Qi.QqQeInf) + "</h3> 次QqQeInfinity<br/>"
        a=a+"你实际上超了 <h3 style='color: #aee308; text-shadow: 0 0 3px #c2b280'>" + format(tmp.Qi.effQqQeInf) + "</h3> 次QqQeInfinity"
        //if(tmp.Qi.effqaqe308.gte(10000)) a=a+'(受软上限限制)'
        a=a+", 使升级E19-3效果的硬上限变为原来的 <h3 style='color: #aee308; text-shadow: 0 0 3px #c2b280'>"+format(tmp.Qi.QqQeInfeff[0])+"</h3> 次方, 也使超前3个人的速度x <h3 style='color: #aee308; text-shadow: 0 0 3px #c2b280'>"+format(tmp.Qi.QqQeInfeff[1])+"</h3> (基于QqQeInfinity效果)" 
        //if(tmp.Qi.qaqe308eff.gte('1e500')) a=a+"(受软上限限制)" 
        a=a+"<br>基于你的QqQeInfinity数量，你的QqQeInfinity每秒超 <h3 style='color: #aee308; text-shadow: 0 0 3px #c2b280'>"+ format(tmp.Qi.QqQeInfspeed) +"</h3> 次QqQeInfinity,"
        a=a+" 相当于每 <h3 style='color: #aee308; text-shadow: 0 0 3px #c2b280'>"+format(n(1).div(tmp.Qi.QqQeInfspeed))+"</h3> 秒超一次QqQeInfinity<br/>超QqQeInfinity次数在第六行层级重置之前保留<br>"}}

        if(hasMilestone('Qi',0)) a=a+'<br/>你最多能同时超'+format(tmp.Qi.maxActive,0)+'个人'}
        return a
    },
    effectDescription(){a='which multiply Super-man speed by '+format(tmp.Qi.effect)+'.'
        if(options.Chinese) a='使超人速度x'+format(tmp.Qi.effect)
        return a
    },
    maxActive(){a=n(1)
    ...
    currentActive(){a=n(0)
        ...
        a=a.add(n(getClickableState('Qi',14)))
        return a
    },
    ...
    QqQe308speed(){a=n(1).div(120)
        ...
        if(hasUpgrade('cf',12)) a=a.times(upgradeEffect('cf',12))
    ...
    QqQe308eff(){b=tmp.Qi.effQqQe308
        ...
        if(b.gte(1e15)) b=n(10).pow(b.log(10).div(15).pow(0.25).times(15))
    ...
    cokecolespeed(){a=n(1).div(1e5)
        ...
        a=a.times(buyableEffect('qa',12))
        if(hasUpgrade('cf',12)) a=a.times(upgradeEffect('cf',12))
        if(hasUpgrade('cf',32)) a=a.times(challengeEffect('MT',13))
    ...
    qaqe308eff(){a=tmp.Qi.effqaqe308.add(1).pow(10)
        if(hasUpgrade('E',92)) a=a.pow(1.5)
        if(a.gte('1e500')) a=n(10).pow(a.log(10).div(500).pow(0.5).times(500))
        return a
    },
    effQqQeInf(){a=player.Qi.QqQeInf.floor()
        return a
    },
    ...
    QqQeInfeff(){a=tmp.Qi.effQqQeInf.add(1).log(10).add(1).log(2).add(1)
        b=tmp.Qi.effect.pow(a.sub(1))
        if(hasUpgrade('E',204)) b=b.pow(2)
        return [a,b]
    },
    effect(){a=n(2).pow(player.Qi.points.sub(1))
        if(hasUpgrade('E',91)) a=n(5).pow(player.Qi.points.sub(1))
        return a
    },
})
